...
const MAX_CONSECUTIVE_HYPHEN_ROWS: u32     =     4;
                    ...
                    if r < -1.0 {
                        ...
                        if r > threshold {
                            ...
                            while index > low_index {
                                ...
                                match current {
                                    Item::Box { width, .. } => sums.width -= width,
                                    Item::Penalty { penalty, flagged, .. } if *penalty < INFINITE_PENALTY && (!flagged || consecutive_flagged < MAX_CONSECUTIVE_HYPHEN_ROWS) => {
